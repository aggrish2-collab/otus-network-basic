...
соответствии с методичкой. Часть 5 (Relay на R2) невозможно выполнить
из-за отсутствия поддержки команды `ipv6 dhcp relay` в Cisco Packet
Tracer - задокументированное ограничение симулятора, не связанное с
ошибками в конфигурации. Это демонстрирует ключевой принцип DHCPv6:
без Relay-агента DHCPv6-сообщения, будучи link-local multicast, не
могут пересечь границу маршрутизатора.